...
const lcarsPanelVariants = cva(
  "rounded-lg font-lcars tracking-wide transition-all duration-300",
  {
    variants: {
      variant: {
        default:
        ...
        gray:
          "bg-lcars-dark border-2 border-lcars-gray text-lcars-gray shadow-[0_0_10px_rgba(128,128,128,0.2),inset_0_0_10px_rgba(128,128,128,0.05)]",
        green:
...
const LCARSPanel = React.forwardRef<HTMLDivElement, LCARSPanelProps>(
  ({ className, variant, size, shape, glow, title, description, children, ...props }, ref) => {
    return (
      ...
      >
        {title && (
          <h3 className="mb-2 text-hierarchy-primary">{title}</h3>
        )}
        {description && (
          <p className="mb-3 text-hierarchy-tertiary">{description}</p>
        )}
        {children}
      </div>
    );
